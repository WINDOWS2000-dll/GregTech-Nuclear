...
import com.windows2000.gtnuclear.api.modules.*;

/**
 * Discovers every {@link IModuleContainer} (annotated with {@link ModuleContainer}, or registered manually via
 * {@link #registerContainer}) and every {@link IModule} (annotated with {@link GTNuclearModule}) on the classpath
 * via
 * {@link ASMDataTable}, resolves inter-module dependencies with a topological sort, lets players individually
 * enable/disable modules via a generated {@code modules.cfg}, and dispatches every FML lifecycle event to the
 * modules that ended up loaded, in dependency order. See {@link IModule} for the events forwarded.
 */
public class ModuleManager implements IModuleManager {
    ...
    private final Logger logger = LogManager.getLogger("GTNuclear Module Loader");
    ...
    public boolean isModuleEnabled(IModule module) {
        GTNuclearModule annotation = module.getClass().getAnnotation(GTNuclearModule.class);
    ...
    public void setup(ASMDataTable asmDataTable, File configDirectory) {
        ...
        configFolder = new File(configDirectory, GTNuclearValues.MODID);
    ...
    private void configureModules(Map<String, List<IModule>> modules) {
        ...
        config.addCustomCategoryComment(MODULE_CFG_CATEGORY_NAME,
                "Module configuration file. Can individually enable/disable modules from this mod and its addons");
    ...
    private Map<String, List<IModule>> getModules(ASMDataTable table) {
        ...
        for (IModule module : instances) {
            GTNuclearModule info = module.getClass().getAnnotation(GTNuclearModule.class);
    ...
    private List<IModule> getInstances(ASMDataTable table) {
        Set<ASMDataTable.ASMData> dataSet = table.getAll(GTNuclearModule.class.getCanonicalName());
        ...
        return instances.stream().sorted((m1, m2) -> {
            GTNuclearModule m1a = m1.getClass().getAnnotation(GTNuclearModule.class);
            GTNuclearModule m2a = m2.getClass().getAnnotation(GTNuclearModule.class);
